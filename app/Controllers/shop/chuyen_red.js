	...
	if (!!data && !!data.name && !!data.otp) {
		if (!validator.isLength(data.name, {min: 3, max: 17})) {
			...
			if(validator.isEmpty(name) ||
				...
				otp.length != 4)
			{
				client.red({notice:{title:'CHUYỂN RIK', text:'Kiểm tra lại các thông tin.!'}});
			}else{
							...
							if (true || data_otp && data.otp == data_otp.code) {
								...
								// }else
								{
									...
									.then(valuesCheck => {
										...
										if (!!to) {
											if (to.id == client.UID) {
												client.red({notice:{title:'CHUYỂN RIK',text:'Bạn không thể chuyển cho chính mình.!!'}});
											}else{
												if (user == null || (user.red-10000 < red)) {
													client.red({notice:{title:'CHUYỂN RIK',text:'Số dư không khả dụng.!!'}});
												}else{
													if (user.block) {
														client.red({notice:{title:'CHUYỂN RIK',text:'Bạn không thể chuyển RIK!!'}});
													}else{
														UserInfo.findOneAndUpdate({id: client.UID}, {$inc:{red:-red}}, function (err, result) {
                                                            if (!!result) {

																client.red({notice:{title:'CHUYỂN RIK', text: 'Giao dịch thành công.!!'}, user:{red:user.red-red}});
														...
														if (!!result2) {
															...
															if (void 0 !== client.redT.users[to.id]) {
																Promise.all(client.redT.users[to.id].map(function(obj){
																	obj.red({notice:{title:'CHUYỂN RIK', text:'Bạn nhận được ' + Helper.numberWithCommas(thanhTien) + ' XU.' + '\n' + 'Từ người chơi: ' + client.profile.name}, user:{red: to.red*1+thanhTien}});
																}));
															}
											...
											client.red({notice:{title:'CHUYỂN RIK',text:'Người dùng không tồn tại.!!'}});
										}
									})
								}
							}else{
